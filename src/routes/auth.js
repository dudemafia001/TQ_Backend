...
router.post("/signup", async (req, res) => {
  ...
  if (!username || !fullName || !mobile || !password) {
    return res.status(400).json({ message: "Username, full name, mobile number, and password are required" });
  }
  ...
  try {
    ...
    const user = new User({ username, fullName, mobile, password: hashedPassword });
    await user.save();
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      const hashedPassword = await bcrypt.hash(password, 10);
      user = new User({ username, fullName, mobile, password: hashedPassword });
      await user.save();
      return res.json({ 
